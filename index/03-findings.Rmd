...
In addition, vectors representations of each tweet can be extracted. For each tweet keyword we summarised all vectors by finding their mean values for each dimension. We then found the pairwise distance measures for these ‘average’ tweets.See in Figure 23 below


```{r Average_Tweet_Distance}
...
df_Prophet_Resultsh = read.csv(df_Prophet_Results_path)
kable(df_Twitter, digit = 7, align = "r", caption = "External Data Sources", 
      format = "latex", longtable = TRUE)
```
...
kable(df_lmod_all, digit = 7, align = "r", caption = "External Data Sources", 
      format = "latex", longtable = TRUE)
```
...
kable(df_lmod_step, digit = 7, align = "r", caption = "External Data Sources", 
      format = "latex", longtable = TRUE)
```
...
```{r}
df_lmod_arima_path = "./data/lmod_arima.csv"
df_lmod_arima = read.csv(df_lmod_arima_path)
kable(df_lmod_arima, digit = 7, align = "r", caption = "Results", 
      format = "latex", longtable = TRUE)
```
...
In addition, we decided to challenge the model by only using lags that we thought to be have a reasonable explanation for their effect. To this end we chose to use lags between 6 and 12 months. Our logic was that it would take time for an increase or decrease in the social media presence of one of the keywords selected to go from an uptick in interest of consumer products to be captured by Scholle’s bag sales.

Table 13 displays the results of our Random Forest model.

```{r}
df_RF_path = "./data/RF.csv"
...
![Random Forest Importance](./figure/RF_Importance.png)

The most important features are displayed at the top of this chart, in this the Google Trends lag 12 salsa was the most important predictor followed closely by the month of the year. This agrees with the importance of monthly seasonality that we observed in the other models.
